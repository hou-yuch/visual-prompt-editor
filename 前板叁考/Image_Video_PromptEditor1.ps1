try {
    ...
    $Script:VideoModePresets = @("文生影片模式", "圖生影片模式", "影片修改/延展模式", "影片轉文字模式", "提示詞優化模式")
    ...
    Function Build-VideoPromptDocument {
        ...
        foreach ($Key in @("Subject", "Scene", "Action", "Story", "CameraMotion", "Composition", "Light", "Material", "Color", "Style", "UseCase", "Ratio", "Duration", "Pace")) {
        ...
        $DefaultTechSpecs = [ordered]@{ resolution = "1920x1080 (1080p)"; aspect_ratio = $Fields.Ratio; frame_rate = "24fps 電影感"; codec = "H.264 / H.265 (HEVC)，封裝 MP4"; bitrate = "1080p 10-20 Mbps，VBR"; color_space = "Rec.709 (HD 標準)" }
        $DefaultReviewSpecs = [ordered]@{ stability = "鏡頭運動平滑，無抖動、flicker 或 artifacts"; temporal_consistency = "角色特徵、物件結構、材質在所有影格間保持穩定"; depth_perspective = "景深與透視符合光學邏輯，視角轉換自然"; modular_consistency = "分鏡間色彩校正、光線與整體風格一致"; hud_ui_design = "若有 HUD/UI，需高解析、高對比，動態背景下仍可讀"; accessibility = "教育或訓練用途需預留字幕與音頻描述空間"; narrative_arc = "開場、發展、高潮、收尾具備連續敘事邏輯" }
        ...
        foreach ($Line in @("主體：$($Fields.Subject)", "場景：$($Fields.Scene)", "動作：$($Fields.Action)", "劇情/事件：$($Fields.Story)", "鏡頭運動：$($Fields.CameraMotion)", "構圖：$($Fields.Composition)", "光線：$($Fields.Light)", "材質/質感：$($Fields.Material)", "色彩：$($Fields.Color)", "風格：$($Fields.Style)", "影片用途：$($Fields.UseCase)", "比例：$($Fields.Ratio)", "時長：$($Fields.Duration)", "節奏：$($Fields.Pace)")) { [void]$Out.AppendLine($Line) }
        [void]$Out.AppendLine("")
    ...
    Function Show-VideoPromptEditor {
        ...
        $Script:VideoTechSpecs = [ordered]@{
            ...
            aspect_ratio = "跟隨主視窗比例設定"
            frame_rate = "24fps 電影感"
            codec = "H.264 / H.265 (HEVC)，封裝 MP4"
            bitrate = "1080p 10-20 Mbps，VBR"
            color_space = "Rec.709 (HD 標準)"
        }
        $Script:VideoReviewSpecs = [ordered]@{
            stability = "鏡頭運動平滑，無抖動、flicker 或 artifacts"
            temporal_consistency = "角色特徵、物件結構、材質在所有影格間保持穩定"
            depth_perspective = "景深與透視符合光學邏輯，視角轉換自然"
            modular_consistency = "分鏡間色彩校正、光線與整體風格一致"
            hud_ui_design = "若有 HUD/UI，需高解析、高對比，動態背景下仍可讀"
            accessibility = "教育或訓練用途需預留字幕與音頻描述空間"
            narrative_arc = "開場、發展、高潮、收尾具備連續敘事邏輯"
        }
        ...
        Function Show-VideoTechDialog {
            ...
            $resolution = Add-DialogComboField $dialog "解析度" 25 @("1280x720 (720p)", "1920x1080 (1080p)", "2560x1440 (2K/QHD)", "3840x2160 (4K UHD)", "4096x2160 (DCI 4K)", "7680x4320 (8K)", "依平台預設") $Script:VideoTechSpecs.resolution
            $aspect = Add-DialogComboField $dialog "畫面比例" 65 @("跟隨主視窗比例設定", "16:9 橫式主流", "9:16 手機短影音", "1:1 社群方形", "4:5 社群貼文", "21:9 電影寬螢幕", "2:1 網站橫幅", "3:4 直式人物") $Script:VideoTechSpecs.aspect_ratio
            $fps = Add-DialogComboField $dialog "影格率" 105 @("23.98fps 電影標準", "24fps 電影感", "25fps PAL / 歐規", "29.97fps 廣播/網路", "30fps 標準網路影片", "50fps 高流暢", "59.94fps 高動態", "60fps 運動/動作場景") $Script:VideoTechSpecs.frame_rate
            $codec = Add-DialogComboField $dialog "編碼與格式" 145 @("H.264，MP4 發佈用", "H.265 / HEVC，MP4 高壓縮高畫質", "AV1，Web 發佈", "ProRes 422 HQ，MOV 後製", "ProRes 4444，MOV 含透明/高階後製", "DNxHR HQX，MOV/MXF 後製", "依平台預設") $Script:VideoTechSpecs.codec
            $bitrate = Add-DialogComboField $dialog "位元率" 185 @("1080p 10-20 Mbps，VBR", "1080p 20-35 Mbps，高品質", "4K 35-68 Mbps，VBR", "4K 68-100 Mbps，高品質", "社群平台自動壓縮最佳化", "後製用高位元率", "依平台預設") $Script:VideoTechSpecs.bitrate
            $colorSpace = Add-DialogComboField $dialog "色彩空間" 225 @("Rec.709 (HD 標準)", "Rec.2020 (4K/HDR 規格)", "DCI-P3 (電影/廣色域)", "sRGB (Web 標準)", "Log / Flat，保留後製調色空間", "HDR10 / HLG，HDR 交付", "依平台預設") $Script:VideoTechSpecs.color_space
        ...
        $ql.Text = "15. 品質要求"
        $ql.Font = $FontLabelV
        $ql.Location = New-Object System.Drawing.Point(15, 548)
        $ql.Size = New-Object System.Drawing.Size(275, 20)
        $left.Controls.Add($ql)
        $quality = New-Object System.Windows.Forms.CheckedListBox
        $quality.Location = New-Object System.Drawing.Point(15, 571)
        $quality.Size = New-Object System.Drawing.Size(275, 105)
        ...
        $nl.Text = "16. 排除條件"
        $nl.Font = $FontLabelV
        $nl.Location = New-Object System.Drawing.Point(310, 548)
        $nl.Size = New-Object System.Drawing.Size(255, 20)
        $left.Controls.Add($nl)
        $negative = New-Object System.Windows.Forms.CheckedListBox
        $negative.Location = New-Object System.Drawing.Point(310, 571)
        $negative.Size = New-Object System.Drawing.Size(255, 105)
...
"@
        ...
        $gen.Add_Click({
            $Fields = @{ Subject = Get-SelectedText $subject; Scene = Get-SelectedText $scene; Action = Get-SelectedText $action; Story = Get-SelectedText $story; CameraMotion = Get-SelectedText $cameraMotion; Composition = Get-SelectedText $composition; Light = Get-SelectedText $light; Material = Get-SelectedText $material; Color = Get-SelectedText $color; Style = Get-SelectedText $style; UseCase = Get-SelectedText $useCase; Ratio = Get-SelectedText $ratio; Duration = Get-SelectedText $duration; Pace = Get-SelectedText $pace }
        ...
        $clearVideo.Add_Click({
            foreach ($Combo in @($subject, $scene, $action, $story, $cameraMotion, $composition, $light, $material, $color, $style, $useCase, $ratio, $duration, $pace)) {
                if ($Combo.Items.Count -gt 0) { $Combo.SelectedIndex = 0 }
                $Combo.Text = ""
            }
            ...
            for ($i = 0; $i -lt $negative.Items.Count; $i++) { $negative.SetItemChecked($i, $true) }
            $Script:VideoTechSpecs.resolution = "1920x1080 (1080p)"
            $Script:VideoTechSpecs.aspect_ratio = "跟隨主視窗比例設定"
            $Script:VideoTechSpecs.frame_rate = "24fps 電影感"
            $Script:VideoTechSpecs.codec = "H.264 / H.265 (HEVC)，封裝 MP4"
            $Script:VideoTechSpecs.bitrate = "1080p 10-20 Mbps，VBR"
            $Script:VideoTechSpecs.color_space = "Rec.709 (HD 標準)"
            $Script:VideoReviewSpecs.stability = "鏡頭運動平滑，無抖動、flicker 或 artifacts"
            $Script:VideoReviewSpecs.temporal_consistency = "角色特徵、物件結構、材質在所有影格間保持穩定"
            $Script:VideoReviewSpecs.depth_perspective = "景深與透視符合光學邏輯，視角轉換自然"
            $Script:VideoReviewSpecs.modular_consistency = "分鏡間色彩校正、光線與整體風格一致"
            $Script:VideoReviewSpecs.hud_ui_design = "若有 HUD/UI，需高解析、高對比，動態背景下仍可讀"
            $Script:VideoReviewSpecs.accessibility = "教育或訓練用途需預留字幕與音頻描述空間"
            $Script:VideoReviewSpecs.narrative_arc = "開場、發展、高潮、收尾具備連續敘事邏輯"
